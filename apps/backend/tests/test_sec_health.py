...
async def test_health_hides_exception_text_but_logs_it(
    ...
    async def _qdrant_down() -> None:
        raise ConnectionError("Error 111 connecting to xxxxx.cloud.qdrant.io:6333")

    monkeypatch.setattr(health_routes, "ping_db", _db_down)
    monkeypatch.setattr(health_routes, "ping_qdrant", _qdrant_down)

    with caplog.at_level(logging.WARNING):
        body = await health_routes.health()

    assert body["status"] == "degraded"
    assert body["services"] == {"database": {"ok": False}, "qdrant": {"ok": False}}
    dumped = json.dumps(body)
    for leak in ("neondb_owner", "cloud.qdrant.io", "RuntimeError", "ConnectionError", "detail"):
        assert leak not in dumped
    # ...nhưng người vận hành vẫn đọc được nguyên nhân trong log phía server.
    assert "neondb_owner" in caplog.text and "cloud.qdrant.io" in caplog.text
...
async def test_health_all_ok(monkeypatch: pytest.MonkeyPatch) -> None:
    for name in ("ping_db", "ping_qdrant"):
