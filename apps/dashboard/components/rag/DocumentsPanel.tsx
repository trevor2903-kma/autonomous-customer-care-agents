...
export function DocumentsPanel() {
  ...
  const docs = useQuery<KnowledgeDocument[], Error>({
    queryKey: ["rag-documents"],
    queryFn: getRagDocuments,
  });
  const info = useQuery<RagInfo, Error>({
    queryKey: ["rag-info"],
    queryFn: getRagInfo,
  });
  ...
  const reindex = useMutation<ReindexResult, Error, void>({
  ...
  return (
    <>
      <section className="overflow-hidden rounded-[12px] border border-line bg-white shadow-soft">
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-line-soft px-[18px] py-3.5">
          <div className="min-w-0">
            ...
            <p className="mt-0.5 text-[12.5px] leading-[1.5] text-faint">
              Nguồn chân lý là thư mục{" "}
              <code className="text-[11.5px]">knowledge/</code> trong repo — sửa
              file <code className="text-[11.5px]">.md</code> rồi nạp lại.
            </p>
          </div>
          <button
        ...
        {info.data && (
          <div className="border-b border-line-soft bg-[#FBFAF7] px-[18px] py-2.5 text-[12.5px] text-faint">
            Collection{" "}
            <code className="text-[11.5px] text-dim">
              {info.data.collection}
            </code>{" "}
            ·{" "}
            <span className="font-medium text-ink">
              {info.data.points_count}
            </span>{" "}
            point ·{" "}
            <span className="font-medium text-ink">
              {docs.data?.length ?? 0}
        ...
        <div className="border-b border-line-soft p-4">
          ...
          >
            ...
            <p className="text-[13.5px] font-medium text-[#78726A]">
              {upload.isPending
                ? `Đang tải lên ${selected?.name ?? "tệp"}…`
          ...
          <p className="mt-2 text-center text-[11.5px] leading-relaxed text-faint">
            Tài liệu tải lên là tạm thời — sẽ mất khi nạp lại từ repo. Tri thức
            lâu dài hãy thêm file vào{" "}
        ...
        {err && (
          <p className="border-t border-line-soft px-[18px] py-2.5 text-[12.5px] text-terracotta">
            Lỗi: {err.message}
          </p>
        )}
        {reindex.data && (
          <p className="border-t border-line-soft px-[18px] py-2.5 text-[12.5px] text-olive">
            Đã nạp {reindex.data.documents} tài liệu → {reindex.data.points}{" "}
            point.
          </p>
        )}
      </section>

      {/* Modal xác nhận nạp lại tri thức */}
      <ConfirmModal
      ...
      {/* Modal xác nhận xoá tài liệu tạm thời */}
      <ConfirmModal
        isOpen={!!docToDelete}
        ...
        message={
          <>
            Bạn có chắc chắn muốn xoá tài liệu tạm thời{" "}
